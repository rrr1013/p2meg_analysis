...
// NaI_B1 専用: ノイズが大きいため aMin 係数を緩和
static const double kNaIB1AMinSigmaFactor = 1.5;
...
static const char* kDefaultOutPath = "data/shapeddata/pulses.csv";
...
static double GetAMinSigmaFactorByCh(const std::string& ch_name) {
    if (ch_name == "NaI_B1") return kNaIB1AMinSigmaFactor;
    return kDefaultAMinSigmaFactor;
}
...
static bool LoadTemplate(const std::string& path, TemplateData& t) {
    ...
    t.ch_name.clear();
    ExtractStringAfterKey(s, "CH", t.ch_name);
    return true;
}
...
// 非整数時刻 s で c[s] = sum_j x[s+j] T[j] を線形補間で評価
static bool ComputeCorrelationAtStartFractional(const std::vector<double>& x,
    ...
    if (s + static_cast<double>(L - 1) > static_cast<double>(N - 1)) return false;

    double acc = 0.0;
    for (int j = 0; j < L; ++j) {
        const double xi = s + static_cast<double>(j);
        const int i0 = static_cast<int>(std::floor(xi));
        const double frac = xi - static_cast<double>(i0);
        ...
        if (i0 == N - 1) {
            ...
            xval = (1.0 - frac) * x[i0] + frac * x[i0 + 1];
        }
        acc += xval * T[j];
    }
    c_out = acc;
...
// 線形補間で、非整数シフト時のテンプレ値を返す
// 物理カットではなく、時刻サブサンプル化のための数値補間
static inline double TemplateAtShiftLinear(const std::vector<double>& T, double u) {
    const int L = static_cast<int>(T.size());
    if (L <= 0) return 0.0;
    if (u < 0.0) return 0.0;

    const int j0 = static_cast<int>(std::floor(u));
    const double frac = u - static_cast<double>(j0);
    if (j0 < 0 || j0 >= L) return 0.0;
    if (j0 == L - 1) return (frac == 0.0) ? T[L - 1] : 0.0;
    return (1.0 - frac) * T[j0] + frac * T[j0 + 1];
}

// 非整数時刻（サブサンプル）で振幅・ベースラインを同時フィット
static bool FitAmplitudesFractional(const std::vector<double>& x, const std::vector<double>& T,
                                    const std::vector<double>& tks, double& b_out,
                                    std::vector<double>& a_out, double ridge) {
    const int N = static_cast<int>(x.size());
    const int M = static_cast<int>(tks.size());
    if (N <= 0 || M <= 0) return false;

    std::vector<std::vector<double>> ATA(M + 1, std::vector<double>(M + 1, 0.0));
    std::vector<double> ATy(M + 1, 0.0);
    ...
    for (int i = 0; i < N; ++i) {
        ...
        ATA[0][0] += 1.0;
        for (int k = 0; k < M; ++k) {
            const double uk = TemplateAtShiftLinear(T, static_cast<double>(i) - tks[k]);
            ATy[k + 1] += uk * yi;
            ATA[0][k + 1] += uk;
            ATA[k + 1][0] += uk;
            for (int l = k; l < M; ++l) {
                const double ul = TemplateAtShiftLinear(T, static_cast<double>(i) - tks[l]);
                ATA[k + 1][l + 1] += uk * ul;
                if (l != k) ATA[l + 1][k + 1] = ATA[k + 1][l + 1];
            }
        }
    ...
    if (!SolveNormalEq(ATA, ATy, sol, ridge)) return false;
    b_out = sol[0];
    a_out.assign(sol.begin() + 1, sol.end());
    return true;
}

// 非整数時刻（サブサンプル）での RSS
static double ComputeRSSFractional(const std::vector<double>& x, const std::vector<double>& T,
                                   const std::vector<double>& tks, double b_fit,
                                   const std::vector<double>& a_fit) {
    const int N = static_cast<int>(x.size());
    const int M = static_cast<int>(tks.size());
    double rss = 0.0;
    for (int i = 0; i < N; ++i) {
        double yhat = b_fit;
        for (int k = 0; k < M; ++k) {
            yhat += a_fit[k] * TemplateAtShiftLinear(T, static_cast<double>(i) - tks[k]);
        }
        const double r = x[i] - yhat;
        rss += r * r;
    }
    return rss;
}
...
static bool RefineTimesAroundAnchors(const std::vector<double>& x,
                                     ...
                                     int half_width,
                                     double ridge,
                                     std::vector<double>& tks_out,
                                     double& b_out,
                                     std::vector<double>& a_out,
                                     double& rss_out) {
    if (anchors.empty()) return false;

    std::vector<double> tks(anchors.begin(), anchors.end());
    std::vector<double> a_fit;
    double b_fit = 0.0;
    if (!FitAmplitudesFractional(x, T, tks, b_fit, a_fit, ridge)) return false;
    double rss = ComputeRSSFractional(x, T, tks, b_fit, a_fit);

    const int kMaxPass = 3;
    for (int pass = 0; pass < kMaxPass; ++pass) {
        bool changed = false;
        for (size_t k = 0; k < tks.size(); ++k) {
            double best_tk = tks[k];
            std::vector<double> best_a = a_fit;
            double best_b = b_fit;
            double best_rss = rss;

            for (int d8 = -half_width * 8; d8 <= half_width * 8; ++d8) {
                const double tk_try = static_cast<double>(anchors[k]) + static_cast<double>(d8) / 8.0;
                bool ok_sep = true;
                for (size_t l = 0; l < tks.size(); ++l) {
                    if (l == k) continue;
                    if (std::fabs(tks[l] - tk_try) < static_cast<double>(minSep)) {
                        ok_sep = false;
                        break;
                    }
                }
                ...
                std::vector<double> tks_try = tks;
                tks_try[k] = tk_try;
                std::vector<double> a_try;
                double b_try = 0.0;
                if (!FitAmplitudesFractional(x, T, tks_try, b_try, a_try, ridge)) continue;
                const double rss_try = ComputeRSSFractional(x, T, tks_try, b_try, a_try);
                if (rss_try < best_rss) {
                    best_tk = tk_try;
                    best_a.swap(a_try);
                    best_b = b_try;
            ...
            if (std::fabs(best_tk - tks[k]) > 1.0e-12) {
                tks[k] = best_tk;
                a_fit.swap(best_a);
                b_fit = best_b;
                rss = best_rss;
...
static void ProcessEvent(long long event_id, const std::vector<double>& y,
    ...
    const double aMinSigmaFactor = GetAMinSigmaFactorByCh(t_full.ch_name);
    const double aMin = aMinSigmaFactor * sigmaA;
    ...
    std::vector<double> tks_full_refined(tks_full.begin(), tks_full.end());
    {
        std::vector<double> tks_refined;
        ...
        if (refined_ok) {
            tks_full_refined.swap(tks_refined);
    ...
    tks_crop.resize(tks_full_refined.size());
    for (size_t k = 0; k < tks_full_refined.size(); ++k) {
        tks_crop[k] = static_cast<int>(std::llround(
            tks_full_refined[k] + static_cast<double>(t_corr.crop_start)));
    }
    ...
    for (size_t k = 0; k < tks_full_refined.size(); ++k) {
        ...
        if (!no_sign_cut) {
            double c_now = 0.0;
            if (ComputeCorrelationAtStartFractional(
                    x, t_corr.T, tks_full_refined[k] + static_cast<double>(t_corr.crop_start), c_now)) {
                keep = (a * c_now > 0.0);
            } else {
                keep = false;
            }
        ...
        double t_start_full = tks_full_refined[k];
        ...
        if (dbg) {
            double cv = 0.0;
            ComputeCorrelationAtStartFractional(
                x, t_corr.T, tks_full_refined[k] + static_cast<double>(t_corr.crop_start), cv);
        ...
        ofs << t_full.run_no << "," << event_id << "," << pulse_index << "," << area
            << "," << std::setprecision(15) << fall_start_sample_raw
            << "," << a << "," << baseline_fit << "\n";
        ++pulse_index;
    }
    if (dbg) {
...
int main(int argc, char** argv) {
    ...
    if (positional.size() >= 4) out_path = positional[3];
    ...
    std::error_code ec;
    const std::filesystem::path out_parent = std::filesystem::path(out_path).parent_path();
    if (!out_parent.empty()) {
        std::filesystem::create_directories(out_parent, ec);
        if (ec) {
            std::cerr << "error: failed to create output dir " << out_parent.string()
                      << ": " << ec.message() << "\n";
            return 1;
        }
    }
