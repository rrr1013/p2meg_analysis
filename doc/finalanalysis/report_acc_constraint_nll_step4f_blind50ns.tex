...
\]
へ拡張した。
この更新後の最終 sensitivity sample では、
同一設定の local-exact run を 8 本
（independent seed の 12500 toy shard）
結合し、background-only pseudo-experiment を合計 100000 本用いた。
...
を課している。
この broad rerun の実測では、単独の 12500 toy shard あたり
\[
t_{\mathrm{calib}} = 5.66\ \mathrm{s},\qquad
t_{\mathrm{sens}} = 7241\ \mathrm{s}
\]
であり、1 本あたり約 121 分を要した。
今回はこれを 8 並列で実行し、
最終的な感度分布のみを後段で統合した。
...
上記の belt 再利用近似を採用した。
本番の broad 100000 toy sample でも追加の validation はしていない。
...
\mathrm{BR}_{90}(84\%) &= 1.35\times10^{-3},\\
\mathrm{BR}_{90}(95\%) &= 1.45\times10^{-3}
\end{align}
であった。
新しい broad scan では、図9右端の高 BR 側成分は
もはや current scan 上端への人工的な pile-up ではなく、
background-only toy に対する上限分布の tail として解釈できる。

\begin{figure}[p]
  \centering
  \includegraphics[width=0.95\linewidth]{sensitivity_br_hist_blind50ns.pdf}
  \caption{BR grid \((0\le\mathrm{BR}\le2.5\times10^{-3},\ \Delta\mathrm{BR}=5\times10^{-5})\) 上で評価した、8 本の互換 local-exact run を結合した 100000 本の background-only sensitivity sample の分布。表示用ヒストグラムの bin 幅は見やすさのため \(1\times10^{-4}\) とし、中央値や quantile は元の \(5\times10^{-5}\) grid 上の toy 値から計算している。緑帯は central 68\% interval、黒実線は median expected upper limit \(\mathrm{BR}_{90}^{\mathrm{sens}}=1.05\times10^{-3}\)、赤破線は dedicated FC scan から採用した observed upper limit \(\mathrm{BR}_{90}=1.04\times10^{-3}\) を示す。}
\end{figure}
...
\begin{itemize}
  ...
  \item \texttt{sensitivity\_br\_scan\_blind50ns\_broad\_100000.txt}
