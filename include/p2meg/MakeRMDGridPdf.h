...
//    を用い、検出器分解能 energy_response(E_res,E_true) で Ee, Eg のみをスメアした
...
//  - 生成側で扱うスメアは Ee, Eg のみ（energy_response に従う）。
//
// 外部入力（include/p2meg から参照）
//  - AnalysisWindow.h       : 解析窓（Ee, Eg, t, theta）
//  - DetectorResolution.h   : 分解能（energy_response, sigma_t）, t_mean,
...
//  - truth_win の Ee/Eg を「解析窓」と見做し、energy_response の 0.1 倍点まで広げた窓で真値をサンプルする
