...
// phi を 0..π の (π/N) 格子へ最近傍丸め
static double QuantizePhi(double phi_raw, int N_theta) {
...
double RMDGridPdf(double Ee, double Eg, double t,
  ...
  // phi は 0..pi に丸めてから最近傍格子点へ
  const int N_theta = GetNTheta();
  const double phi_e_q = QuantizePhi(phi_detector_e, N_theta);
  const double phi_g_q = QuantizePhi(phi_detector_g, N_theta);

  const double theta_eg = std::fabs(phi_e_q - phi_g_q);

  // 解析窓（4D: Ee,Eg,t,theta_eg）でカット
  if (!IsInsideWindow4D(Ee, Eg, t, theta_eg)) return 0.0;

  // phi 軸は「最近傍θ格子」になるよう設定されている想定なので FindBin で最近傍に落ちる
  const TAxis* axPe = gHist->GetAxis(2);
  const TAxis* axPg = gHist->GetAxis(3);
  const int bin_pe = axPe->FindBin(phi_e_q);
  const int bin_pg = axPg->FindBin(phi_g_q);
